    ...
    private Double calculateDistanceIncrement(Train t) {
        ...
        switch (t.getStatus()) {
            ...
            case "W DRODZE":
                ...
                String endTime;
                if (t.getLastStopSchedule() == null) {
                    endTime = t.getFirstSchedule().arrival_time;
                } else {
                    endTime = t.getLastStopSchedule().estimatedTime;
...
// Konwersja na typ double przed wykonaniem dzielenia
    ...
    public Double getAverageSpeed(){
        Double result = 0.0;
        Double sumRawSpeed = 0.0; // holds sum of all best train metres by sec. speed if not on station
        Integer numberOfTrainsOnRoad = 0; // number of trains on road "W DRODZE"
        for (int i = 0; i <bestTrainslist.size() ; i++) {
            if(bestTrainslist.get(i).status.equals("W DRODZE") && distanceIncrement.get(i)>0){
                sumRawSpeed = sumRawSpeed + distanceIncrement.get(i);
                numberOfTrainsOnRoad++;
            }
        }
        result = (double)sumRawSpeed/numberOfTrainsOnRoad;
        return result;
    }
